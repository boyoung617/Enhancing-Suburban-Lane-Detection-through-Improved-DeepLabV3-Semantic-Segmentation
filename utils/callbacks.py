...
class EvalCallback():
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            _, IoUs, mpas, accs = compute_mIoU(gt_dir, pred_dir, self.image_ids, self.num_classes, None)  # 执行计算mIoU的函数
            temp_miou = np.nanmean(IoUs) * 100
            temp_MPA  = np.nanmean(mpas) * 100
            temp_acc  = np.nanmean(accs) * 100
            ...
            plt.figure()
            plt.plot(self.epoches, self.mpas, 'red', linewidth=2, label='train mpa')
            plt.grid(True)
            plt.xlabel('Epoch')
            plt.ylabel('MPA')
            ...
            plt.figure()
            plt.plot(self.epoches, self.accs, 'red', linewidth=2, label='train accuracy')
            plt.grid(True)
            plt.xlabel('Epoch')
            plt.ylabel('Accuracy')
